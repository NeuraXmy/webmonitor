<template>
    <div>
      <el-breadcrumb>
          ...
          <el-breadcrumb-item>用户空间列表</el-breadcrumb-item>
      </el-breadcrumb>
      <el-card>
          <el-row>
              <el-col :span="2">
  ...
  export default{
      ...
      methods:{
          async getMonitorSpaceList(){
              ...
              const {data: res} = await this.$axios.get('/user/' + this.UserID + '/spaces',
              {
          ...
          addSpaceList(){
            this.$refs.addSpaceRef.validate(async valid => {
                ...
                const {data: res} = await this.$axios.post('/user/' + this.UserID + '/space',data,
                {
                    headers : {
                        'token': sessionStorage.getItem('token')
                    }
          ...
          async ConfirmDeleteSpace(){
            ...
            const {data: res} = await this.$axios.put('/space/'+this.EditSpaceID+'/softdelete',data,
            {
